...
bool isPieceMoveValid(char board[BOARD_SIZE][BOARD_SIZE], const std::string& source, const std::string destination, const std::string& currentPlayer) {
    ...
    switch (piece) {
        case 'P':  // White Pawn
            ...
            // Capture diagonally
            if ((std::abs(destFile - sourceFile) == 1) && (std::abs(sourceRank - destRank) == 1) && (islower(destPiece))) {
                return true;
            }
            break;
        case 'p':  // Black Pawn
            ...
            // Capture diagonally
            if ((std::abs(sourceFile - destFile) == 1) && (std::abs(destRank - sourceRank) == 1) && (isupper(destPiece))) {
                return true;
            }
            break;
        case 'R':  // Rook
